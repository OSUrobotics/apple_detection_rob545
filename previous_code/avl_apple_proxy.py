...
def main():
    try:
        ...
        for j in range(2, len(real_pick_angles)):
            ...
            satisfied = 'no'
            while not(satisfied == 'yes'):
                print("Scan the stem in the proxy and check if it is fine")

                # apple_proxy_experiment.scan_apple_and_stem()

                print("Result: current Stem-Gravity angle in proxy is %.0f deg: " % apple_proxy_experiment.stem_to_gravity)
                print("Are you satisfied? 'yes' or 'no' ")
                satisfied = raw_input()
            ...
            if angles:
                for noise in range(5):
                    ...
                    rdm_x = np.random.randint(0, 5)
                    rdm_y = np.random.randint(0, 6)
                    rdm_z = np.random.randint(0, 3)
                    ...
                    rdm_roll = np.random.randint(0, 5)
                    rdm_pitch = np.random.randint(0, 2)

                    roll_noise = math.radians(roll_noise_range[rdm_roll])       # [rad]
                    pitch_noise = math.radians(pitch_noise_range[rdm_pitch])    # [rad]

                    print("The angular noises added are %.3f and %.3f" % (roll_noise, pitch_noise))
                    apple_proxy_experiment.add_angular_noise(roll_noise, pitch_noise, 0)


                    # --- Arrange Rosbag file and subscribe to the topics that you want to record
                    rosbag_name = ''
                    sub_name = str(int(index)) + "-" + str(noise)
                    command = "rosbag record -O " + "/home/avl/ur_ws/src/apple_proxy/bag_files/apple_proxy_pick" \
